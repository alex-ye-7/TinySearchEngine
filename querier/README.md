...
I am using my 48 hour extention on this assignment. 

# Assumptions and Differences

There were no major assumptions that I made in addition to the assumptions already mentioned in the Specs. 

There is only one big difference between my solution and Specs. I chose a different way to chop the query line that involves trimming the leading and trailing spaces, using strchr to search for the first non-leading space to divide the words up, and reallocating the size of my words array for each new word.

# Functionality

Based on the functinality scoring scale, I believe I reached a solution that prints the document set in decreasing order by score, implmeneting the full functinality described in the Specs. 

Below you can find the Tiny Search Engine Querier descirption. 
