...
# Assumptions
One assumption made is that pageDirectory passed into the command line does not already contain any files whose name is an integer (i.e., 1, 2, …). There were no other major assumptions that deviated from the three Specs provided. 

The directions for the crawler assignment begin below:
